<?php
	class Order extends CI_Controller{
		...
		public function  pay(){		
			...
			$input = array('order_date'=>$order_date,'order_time'=>$order_time,'id_costumer'=>$id_costumer,'id_promo_code'=>$id_promo_code,'final_price'=>$final_price,'id_payment_type'=>$payment_type,'buyer_name'=>$buyer_name,'buyer_phone'=>$buyer_phone,'buyer_email'=>$buyer_email,'status'=>$status,'check_in'=>'Belum CekIn','bukti_transfer'=>'noname');
		    ...
		    for ($i=1; $i <= $jml ; $i++) {
		    	...
		    	$ticket_code = 'TG'.strtoupper(ranS(9,'all')); //INi kode random ticket
			...
			if ($tipebayar == '3') {
				$this->m_general->uData('costumer',array('Balance'=> $BLC- $final_price ),array('id_costumer'=>$id_costumer)); //Kok Gabisa Anjir
			}
